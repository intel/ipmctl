  ...
  )
{
  ...
  for (Index = 0; Index < pDimm->FmtInterfaceCodeNum; Index++) {
    if (AEP_FMT_CODE_APP_DIRECT == pDimm->FmtInterfaceCode[Index] ||
        AEP_FMT_CODE_STORAGE == pDimm->FmtInterfaceCode[Index]) {
      Supported = TRUE;
      break;
    }
  }
  ...
  )
{
  ...
  UINT32 Offset = 0;
  UINT32 BytesLeft = 0;
  UINT32 BytesRetrieved = 0;
  PT_INPUT_PAYLOAD_GET_FAILURE_ANALYSIS_DATA InputPayload;
  PT_OUTPUT_PAYLOAD_GET_FA_BLOB_HEADER FABlobHeader;

  NVDIMM_ENTRY();

  ZeroMem(&InputPayload, sizeof(InputPayload));
  ZeroMem(&FABlobHeader, sizeof(FABlobHeader));

  if (pDimm == NULL || ppOutputBuffer == NULL || pBytesWritten == NULL) {
    goto Finish;
  }
  ...
  ReturnCode = FwCmdGetFABlobHeader(pDimm, ID, &FABlobHeader);
  if (EFI_ERROR(ReturnCode)) {
    NVDIMM_WARN("Failed to get failure analysis blob header, TokenID = %d", ID);
    goto Finish;
  }

  *ppOutputBuffer = AllocateZeroPool(sizeof(FABlobHeader) + FABlobHeader.Size);
  ...
  CopyMem((UINT8 *) *ppOutputBuffer, &FABlobHeader, sizeof(FABlobHeader));
  *pBytesWritten = sizeof(FABlobHeader);

  InputPayload.Action = ActionGetFABlobSmallPayload;
  InputPayload.ID = ID;

  pFwCmd->DimmID = pDimm->DimmID;
  pFwCmd->Opcode = PtGetLog;
  pFwCmd->SubOpcode = SubopFailureAnalysis;
  pFwCmd->InputPayloadSize = sizeof(InputPayload);
  pFwCmd->OutputPayloadSize = OUT_PAYLOAD_SIZE;

  for (Offset = 0; Offset < FABlobHeader.Size; Offset += OUT_PAYLOAD_SIZE) {
    InputPayload.Offset = Offset;
    CopyMem(pFwCmd->InputPayload, &InputPayload, pFwCmd->InputPayloadSize);
    ReturnCode = PassThru(pDimm, pFwCmd, PT_TIMEOUT_INTERVAL);
    if (EFI_ERROR(ReturnCode)) {
      NVDIMM_WARN("Failed to get failure analysis blob, TokenID = %d", ID);
      if (FW_ERROR(pFwCmd->Status)) {
        ReturnCode = MatchFwReturnCode(pFwCmd->Status);
      }
      goto Finish;
    ...
    /* Data might not be aligned to payload size, don't copy the extra bytes */
    BytesLeft = FABlobHeader.Size - InputPayload.Offset;
    if (BytesLeft < OUT_PAYLOAD_SIZE) {
      BytesRetrieved = BytesLeft;
    } else {
      BytesRetrieved = OUT_PAYLOAD_SIZE;
    }

    CopyMem((UINT8 *) *ppOutputBuffer + *pBytesWritten, pFwCmd->OutPayload, BytesRetrieved);
    *pBytesWritten += BytesRetrieved;
  }
  ...
  )
{
  ...
  EFI_STATUS ReturnCode = EFI_INVALID_PARAMETER;
  PT_INPUT_PAYLOAD_GET_FAILURE_ANALYSIS_DATA InputPayload;
  PT_OUTPUT_PAYLOAD_GET_FA_INVENTORY *pOutputPayload = NULL;

  NVDIMM_ENTRY();

  ZeroMem(&InputPayload, sizeof(InputPayload));

  if (pDimm == NULL || pMaxFATokenID == NULL) {
    goto Finish;
  }
  ...
  InputPayload.Action = ActionGetFAInventory;

  pFwCmd->DimmID = pDimm->DimmID;
  pFwCmd->Opcode = PtGetLog;
  pFwCmd->SubOpcode = SubopFailureAnalysis;
  pFwCmd->InputPayloadSize = sizeof(InputPayload);
  pFwCmd->OutputPayloadSize = sizeof(PT_OUTPUT_PAYLOAD_GET_FA_INVENTORY);

  CopyMem(pFwCmd->InputPayload, &InputPayload, pFwCmd->InputPayloadSize);

  ReturnCode = PassThru(pDimm, pFwCmd, PT_TIMEOUT_INTERVAL);
  if (EFI_ERROR(ReturnCode)) {
    NVDIMM_WARN("Failed to get failure analysis inventory (RC = %r)",
        ReturnCode);
    NVDIMM_WARN("FW CMD Status %d", pFwCmd->Status);
    if (FW_ERROR(pFwCmd->Status)) {
      ReturnCode = MatchFwReturnCode(pFwCmd->Status);
    }
    goto Finish;
  ...
  pOutputPayload = (PT_OUTPUT_PAYLOAD_GET_FA_INVENTORY *) pFwCmd->OutPayload;
  *pMaxFATokenID = pOutputPayload->MaxFATokenID;
...
/**
  Firmware command to get failure analysis blob header
  ...
  )
{
  ...
  if (pDimm == NULL || pFABlobHeader == NULL) {
    goto Finish;
  }
  ...
  InputPayload.Action = ActionGetFABlobHeader;
  InputPayload.ID = ID;
  ...
  pFwCmd->SubOpcode = SubopFailureAnalysis;
  pFwCmd->InputPayloadSize = sizeof(InputPayload);
  pFwCmd->OutputPayloadSize = sizeof(*pFABlobHeader);

  CopyMem(pFwCmd->InputPayload, &InputPayload, pFwCmd->InputPayloadSize);

  ReturnCode = PassThru(pDimm, pFwCmd, PT_TIMEOUT_INTERVAL);
  if (EFI_ERROR(ReturnCode)) {
    NVDIMM_WARN("Failed to get failure analysis blob header for TokenID: %d (RC = %r)",
        ID, ReturnCode);
    NVDIMM_WARN("FW CMD Status %d", pFwCmd->Status);
    if (FW_ERROR(pFwCmd->Status)) {
      ReturnCode = MatchFwReturnCode(pFwCmd->Status);
  ...
  CopyMem(pFABlobHeader, pFwCmd->OutPayload, sizeof(*pFABlobHeader));
